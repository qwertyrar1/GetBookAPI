...
@pytest.fixture
async def get_book_from_database(asyncpg_pool):

    async def get_book_from_database_by_uuid(book_id: str):
        async with asyncpg_pool.acquire() as connection:
            return await connection.fetch("""SELECT * FROM books WHERE id = $1;""", book_id)

    return get_book_from_database_by_uuid
...
async def create_book_in_database(asyncpg_pool):
    async def create_book_in_database_by_data(id: str, name: str, download_link: str):
        async with asyncpg_pool.acquire() as connection:
            return await connection.execute("""INSERT INTO books VALUES ($1, $2, $3)""",
                                            str(id), name, download_link)
